...
bool ASpawnBox::SpawnActor()
{
	...
	if (ActorClassToBeSpawned)
	{
		...
		SpawnLocation.Y += -BoxBounds.BoxExtent.X + 2 * BoxBounds.BoxExtent.Y * FMath::FRand();
		SpawnLocation.Z += -BoxBounds.BoxExtent.X + 2 * BoxBounds.BoxExtent.Z * FMath::FRand();
...
void ASpawnBox::EnableSpawningActor(bool Enable)
{
	// Update internal state
	ShouldSpawn = Enable;

	// Check whitch timer action should be done
	if (Enable)
	{
		ScheduleActorSpawn();
	}
